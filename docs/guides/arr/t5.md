...
- Firestorm: Marks a target with a blue marker for 3s, then traps them and everyone nearby in a Conflagration circle (faces south, same size as the boss's hitbox). Does not target the first/second enmity. Going in causes you to also be trapped. Trapped people suffer a weaker Burns DoT, but are otherwise immune to damage. Conflagration must be killed quickly or it will explode, dealing massive damage to everyone trapped inside (oneshots squishies)
